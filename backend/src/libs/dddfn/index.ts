...
export type Validator<T, Token extends string, E extends Error> = Nominal<(ctx: T) => E[], `validator_${Token}`>
export const Validator = <T, E extends Error, Token extends string = "">(
  validator: (ctx: T) => E[],
): Validator<T, Token, E> => {
  return validator as Validator<T, Token, E>
}

export type PermissionChecker<T, Token extends string, E extends Error> = Nominal<(ctx: T) => E[], `permissionChecker_${Token}`>
export const PermissionChecker = <T, E extends Error, Token extends string = "">(
  checker: (ctx: T) => E[],
